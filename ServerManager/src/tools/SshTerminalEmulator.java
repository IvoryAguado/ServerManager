 ...
 *
 */
public class SshTerminalEmulator
{
	...
	private static Server serverD=null;
	 ...
	 *
	 */
	static class TerminalDialog extends JDialog
	{
		private static final long serialVersionUID = 1L;
	...
	class AdvancedVerifier implements ServerHostKeyVerifier
	{
				...
				byte[] serverHostKey) throws Exception
		{
			...
			int choice = JOptionPane.YES_OPTION;//JOptionPane.showConfirmDialog(loginFrame, message);
	...
	class InteractiveLogic implements InteractiveCallback
	{
		...
		/* We tain a prompt counter - this enables the detection of situations where the ssh
	...
	void showGUI()
	{
		...
		loginFrame = new JFrame("SSH2 Terminal Emulator - SmorenburgDS");
		loginFrame.setBackground(new Color(0, 102, 255));
	...
	public static void main(String[] args8, Server server)
	{
		serverD=server;
		
		SshTerminalEmulator client = new SshTerminalEmulator();
		client.startGUI();
	}
